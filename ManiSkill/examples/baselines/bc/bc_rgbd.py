...
class ManiSkillDataset(Dataset):
    def __init__(self, dataset_file: str, load_count) -> None:
        self.dataset_file = dataset_file
        # Open file and load JSON metadata
        ...
        for eps_id in tqdm(range(load_count)):
            ...
            # Only load RGB observations (ignore depth)
    ...
    def __getitem__(self, idx):
        out = {}
        # Keep data on CPU (do not call .to(device))
        out["action"] = torch.from_numpy(self.actions[idx]).float()
        rgb = torch.from_numpy(self.rgb[idx]).float()
        out["rgb"] = rgb
        out["state"] = torch.from_numpy(self.states[idx]).float()
        return out


# IterationBasedBatchSampler as provided
...
class PlainConv(nn.Module):
    def __init__(self, in_channels=3, out_dim=256, max_pooling=True, inactivated_output=False):
        """
        Note: in_channels is set to 3 now, since we use only RGB.
        """
        super().__init__()
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        super().__init__()
        # Use 3 input channels for RGB only.
    ...
    def forward(self, rgb, state):
        # Expecting "rgb" key only (no depth concatenation)
...
if __name__ == "__main__":
    ...
    # Environment setup: use only RGB
    env_kwargs = dict(
    ...
    # Do not pass a device to the dataset so that data stays on CPU.
    ...
    for iteration, batch in enumerate(data_loader):
        # Move the batch from CPU to GPU here
        ...
        if iteration % args.eval_freq == 0:
            actor.eval()
            # Update normalization tensor for only RGB
            norm_tensor = torch.Tensor([255.0, 255.0, 255.0]).to(device)

            def sample_fn(obs):
                if isinstance(obs["rgb"], np.ndarray):
                    for k, v in obs.items():
                        obs[k] = torch.from_numpy(v).float().to(device)
                obs["rgb"] = torch.div(obs["rgb"], norm_tensor)
                action = actor(obs["rgb"], obs["state"])
