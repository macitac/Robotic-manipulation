...
def load_ckpt(checkpoint_path, device):
    checkpoint = torch.load(checkpoint_path, map_location=device)
    actor.load_state_dict(checkpoint["actor"])
    start_iter = checkpoint.get("iteration", 0) + 1
    print("Model statistics after loading checkpoint:")
    print_model_stats(actor)
    return start_iter

def save_ckpt(run_name, tag=None):
    if tag is None:
        tag = "final"
...
def benchmark_actor_inference(actor, env, device, obs_horizon, num_runs=100, batch_size=1):
    """
    Benchmark the inference speed of the actor using a real observation from the environment.
    The observation is repeated along the time dimension (obs_horizon) to match the actor's expected input shape.
    """
    print(f"\nRunning actor inference benchmark with batch_size={batch_size} using real data...")
    
    # Get a real observation from the environment
    obs, _ = env.reset()
    
    # Build a new observation dictionary (obs_seq) that matches the expected input shape.
    # For visual data (rgb/depth), we assume env.reset() returns shape (H, W, C) or (B, H, W, C).
    # We convert them to tensors of shape (B, obs_horizon, C, H, W).
    obs_seq = {}
    for key, value in obs.items():
        # Convert to tensor if needed and send to device.
        if not isinstance(value, torch.Tensor):
            value = torch.tensor(value)
        value = value.to(device)
        
        if key in ["rgb", "depth"]:
            # If the observation is a single frame with shape (H, W, C)
            if value.ndim == 3:
                # Add a batch dimension
                value = value.unsqueeze(0)  # shape becomes (1, H, W, C)
            # Now add a time dimension by repeating the same frame.
            if value.ndim == 4:
                # Expand to (B, obs_horizon, H, W, C) and then permute to (B, obs_horizon, C, H, W)
                value = value.unsqueeze(1).repeat(1, obs_horizon, 1, 1, 1)
                value = value.permute(0, 1, 4, 2, 3)
            elif value.ndim == 5:
                # If already with a time dimension, simply ensure channels are in the right position.
                value = value.permute(0, 1, 4, 2, 3)
        else:
            # For state information, assume shape is (state_dim,) or (B, state_dim).
            if value.ndim == 1:
                # Convert to shape (B, obs_horizon, state_dim) with B=batch_size.
                value = value.unsqueeze(0).unsqueeze(0).repeat(batch_size, obs_horizon, 1)
            elif value.ndim == 2:
                # If already (B, state_dim), add time dimension.
                value = value.unsqueeze(1).repeat(1, obs_horizon, 1)
        obs_seq[key] = value

    # Warm up the GPU to avoid any startup overhead.
    for _ in range(10):
        with torch.no_grad():
    ...
    print(f"\nActor Inference Benchmark Results:")
...
if __name__ == "__main__":
    ...
    # Define dummy_obs for debugging after actor is defined
    dummy_obs = {
        "rgb": torch.randint(0, 256, (1, args.obs_horizon, 3, 64, 64), dtype=torch.uint8).to(device),
        "depth": torch.rand(1, args.obs_horizon, 1, 64, 64).to(device),
        "state": torch.rand(1, args.obs_horizon, state_dim, device=device)
    }
    ...
    start_iter = 0
    # --- RESUME BLOCK: if resuming, load checkpoint and immediately evaluate ---
    if args.resume and args.resume_path is not None and os.path.exists(args.resume_path):
        start_iter = load_ckpt(args.resume_path, device)
        print(f"Resumed from checkpoint at iteration {start_iter - 1}.")

        # Debug checks after actor and dummy_obs are defined:
        print("Checkpoint keys:", torch.load(args.resume_path, map_location=device).keys())
        with torch.no_grad():
            dummy_output = actor(dummy_obs)
        print("Output on dummy observation:", dummy_output)
        ...
        # Call the benchmark function directly after evaluation.
        avg_time, fps = benchmark_actor_inference(actor, envs, device, obs_horizon=args.obs_horizon, num_runs=100, batch_size=1)
