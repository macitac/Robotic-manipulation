...
@register_env("StackCube-v1-new", max_episode_steps=50)
class StackCubeEnv(BaseEnv):
    ...
    **Success Conditions:**
    ...
    def _default_sensor_configs(self):
        ...
        base_pose = Pose.create(base_pose)

        # Fixed translation offset (same for all envs)
        position_offset = torch.tensor([0.02, -0.01, 0.01], device=self.device).expand(self.num_envs, -1)

        # Fixed rotation: 5 degrees (pi/36) around Z-axis
        angle = torch.tensor(np.pi / 36, device=self.device)
        sin = torch.sin(angle / 2)
        cos = torch.cos(angle / 2)
        axis = torch.tensor([0.0, 0.0, 1.0], device=self.device)
        quat = torch.cat([
            cos.expand(self.num_envs, 1),              # w
            (axis * sin).expand(self.num_envs, -1)     # x, y, z
        ], dim=1)  # shape: (num_envs, 4)

        # Apply pose offset
        offset_pose = Pose.create_from_pq(p=position_offset, q=quat)
        pose = base_pose * offset_pose
    ...
    def _load_lighting(self, options: dict):
        ...
        self.scene.set_ambient_light([0.7, 0.7, 0.3])
