...
class UnitreeG1UpperBody(BaseAgent):
    ...
    def _after_init(self):
        ...
        # Disable collisions between fingers.
    ...
    def right_hand_dist_to_open_grasp(self):
        """Compute the distance from the current qpos to an open grasp qpos for the right hand."""
        return torch.mean(
            torch.abs(self.robot.qpos[:, self.right_finger_joint_indexes]), dim=1
        )

    def left_hand_dist_to_open_grasp(self):
        """Compute the distance from the current qpos to an open grasp qpos for the left hand."""
        return torch.mean(
    ...
    def left_hand_is_grasping(self, object: Actor, min_force=0.5, max_angle=85):
        ...
        ldirection = self.left_hand_finger_link_l_1.pose.to_transformation_matrix()[..., :3, 1]
        rdirection1 = -self.left_hand_finger_link_r_1.pose.to_transformation_matrix()[..., :3, 1]
        rdirection2 = -self.left_hand_finger_link_r_2.pose.to_transformation_matrix()[..., :3, 1]
        ...
        rflag1 = torch.logical_and(rforce_1 >= min_force, torch.rad2deg(rangle1) <= max_angle)
        rflag2 = torch.logical_and(rforce_2 >= min_force, torch.rad2deg(rangle2) <= max_angle)
        rflag = rflag1 | rflag2
    ...
    def right_hand_is_grasping(self, object: Actor, min_force=0.5, max_angle=85):
        ...
        ldirection = self.right_hand_finger_link_l_1.pose.to_transformation_matrix()[..., :3, 1]
        rdirection1 = -self.right_hand_finger_link_r_1.pose.to_transformation_matrix()[..., :3, 1]
        rdirection2 = -self.right_hand_finger_link_r_2.pose.to_transformation_matrix()[..., :3, 1]
...
@register_agent()
class UnitreeG1UpperBodyWithHeadCamera_2(UnitreeG1UpperBody):
    uid = "unitree_g1_simplified_upper_body_with_head_camera_2"

    @property
    def _sensor_configs(self):
        # Return four camera configurations:
        # 2 high-resolution cameras mounted on the head and
        # 2 wrist cameras mounted on the left and right TCP links.
        return [
            CameraConfig(
                uid="cam_left_high",
